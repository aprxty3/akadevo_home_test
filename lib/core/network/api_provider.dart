...
import '../../utils/generic_widget/common_widget.dart';
import '../../utils/service/storage.dart';
...
class APIProvider {
  ...
  APIProvider() {
    ...
    _dio.interceptors
      ...
      //     JwtDecoder.isExpired(storage.getString(StorageConstants.token)!)) {
      //   Get.log(
      //       'isExpired Token : ${JwtDecoder.isExpired(storage.getString(StorageConstants.token)!)}');
      //   Get.log(
      //       'refresh_token :${storage.getString(StorageConstants.refreshToken)}');
      //   Get.log('last Token :${storage.getString(StorageConstants.token)}');
      //   _dio.lock();
      //
      //   await tokenDio.post('$_baseUrl/sanctum/token', data: {
      //     "username": '${storage.getString(StorageConstants.username)}',
      //     "password": '${storage.getString(StorageConstants.password)}',
      //     "tokenName": '${storage.getString(StorageConstants.deviceId)}',
      //     "unique": true
      //   }).then((response) async {
      //     Get.log(
      ...
      //         Get.offAllNamed(SignInPage.namePath);
  ...
  Future<Either<GenericException, Response>> postDataImage(
      String path, String titleName, FormData imageData) async {
    Get.log('dataaa ${imageData.files.first.value}');
    try {
      _dio.options.headers.addAll({'Content-Type': 'multipart/form-data'});
      var response = await _dio.post(
        path,
        data: {"name": titleName, "file": imageData.files.first.value},
